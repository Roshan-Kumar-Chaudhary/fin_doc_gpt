...
#             # Sentiment metric with proper delta_color values
#             if 'error' not in sentiment_data:
#                 sentiment_score = sentiment_data['score']
#                 sentiment_label = "Bullish" if sentiment_score > 0.2 else "Bearish" if sentiment_score < -0.2 else "Neutral"
                
#                 # Determine correct delta_color value
#                 if sentiment_score > 0.2:
#                     delta_color = "normal"  # Will show green
#                 elif sentiment_score < -0.2:
#                     delta_color = "inverse"  # Will show red
#                 else:
...
#                     "Market Sentiment",
#                     f"{sentiment_label} ({sentiment_score:.2f})",
#                     delta=f"{sentiment_data['summary']['positive']}👍 / {sentiment_data['summary']['negative']}👎",
#                     delta_color=delta_color
#                 )
#             else:
#                 cols[4].metric("Market Sentiment", "No data")
...
#                     title="Sentiment Distribution in Recent News"
#                 )
#                 st.plotly_chart(fig, use_container_width=True)
                
#                 # News articles with sentiment
#                 st.markdown("**Recent News Articles**")
#                 for article in sentiment_data['articles'][:3]:  # Show top 3
#                     with st.expander(f"{article['title']} ({article['source']})"):
#                         col1, col2 = st.columns([4, 1])
#                         col1.write(f"Published: {article.get('time_published', 'N/A')}")
#                         sentiment = article['sentiment']
#                         if sentiment['label'] == "POSITIVE":
#                             col2.success(f"Sentiment: {sentiment['label']} ({sentiment['score']:.2f})")
#                         elif sentiment['label'] == "NEGATIVE":
#                             col2.error(f"Sentiment: {sentiment['label']} ({sentiment['score']:.2f})")
#                         else:
#                             col2.info(f"Sentiment: {sentiment['label']} ({sentiment['score']:.2f})")
#                         st.markdown(f"[Read more]({article['url']})")
            
#             # Forecasting section
#             st.subheader("Forecasting Tools")
            
#             forecast_col, prob_col = st.columns(2)
...
#                         try:
#                             # Include sentiment in forecast if available
#                             sentiment_weight = sentiment_data.get('score', 0) if 'error' not in sentiment_data else 0
#                             forecast = forecaster.forecast_prophet(history, forecast_days, sentiment_weight)
...
#                             # Add sentiment annotation if significant
#                             if abs(sentiment_weight) > 0.3:
#                                 annotation_text = "Bullish Sentiment Impact" if sentiment_weight > 0 else "Bearish Sentiment Impact"
#                                 fig.add_annotation(
...
#                             if change_pct > 0:
#                                 st.success(f"Projected {forecast_days}-day change: +{change_pct:.2f}%")
#                             else:
#                                 st.error(f"Projected {forecast_days}-day change: {change_pct:.2f}%")
...
#             with prob_col:
#                 st.markdown("**Next-Day Movement Prediction**")
#                 if st.button("Predict Tomorrow's Movement"):
#                     with st.spinner("Analyzing market patterns..."):
#                         try:
#                             # Include sentiment in prediction
#                             sentiment_weight = sentiment_data.get('score', 0) if 'error' not in sentiment_data else 0
#                             move_prob, explanation, importance = forecaster.forecast_movement(history, sentiment_weight)
                            
#                             # Display prediction with sentiment context
#                             if move_prob > 0.6:
#                                 st.success(f"📈 Likely to increase ({move_prob:.0%} confidence)")
#                                 if sentiment_weight > 0.2:
#                                     st.info("Positive market sentiment supports this prediction")
#                             elif move_prob < 0.4:
#                                 st.error(f"📉 Likely to decrease ({(1-move_prob):.0%} confidence)")
#                                 if sentiment_weight < -0.2:
#                                     st.info("Negative market sentiment supports this prediction")
#                             else:
#                                 st.info(f"🤷 Uncertain movement ({move_prob:.0%} confidence)")
